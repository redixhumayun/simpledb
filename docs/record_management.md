...
## True Variable-Length Field Implementation

Tracking Issue: [#64](https://github.com/redixhumayun/simpledb/issues/64)

**Status: Implemented** (branch `feature/var-len-tuple-heaps`, completed 2026-02-22).

### Dense Tuple Format

Each heap tuple in the record space has the following binary layout:

```
[HeapTupleHeader (24 bytes)][null_bitmap (ceil(n/8) bytes)][field data...]
```

**HeapTupleHeader** (24 bytes):
```
Bytes  0.. 3: payload_len (u32 LE) — byte count of [null_bitmap + field data], NOT including slack
Bytes  4..11: xmin (u64 LE)        — creating transaction id (0 for now)
Bytes 12..19: xmax (u64 LE)        — deleting/updating transaction id (0 for now)
Bytes 20..21: flags (u16 LE)
Bytes 22..23: nullmap_ptr (u16 LE) — offset of null bitmap within payload (always 0)
```

**Null bitmap**: `ceil(n_fields / 8)` bytes immediately after the header. Bit `i` is set (1) if field `i` is NULL.

**Field data** (after null bitmap, in schema order):
- `NULL` fields: zero bytes
- `INT` fields: 4 bytes, little-endian `i32`
- `STRING` fields: 4-byte LE `u32` length prefix + actual UTF-8 bytes (no padding)

**Example**: Schema `[id: INT, name: VARCHAR(100), age: INT]`, values `[42, "alice", 30]`

```
Before (fixed slots):  24B header | 4B nullmap | 4B(42) | 4B(5) + 5B("alice") + 95B(pad) | 4B(30) = 136 bytes
After  (dense packing): 24B header | 1B nullmap | 4B(42) | 4B(5) + 5B("alice") | 4B(30)  =  38 bytes
```

**NULL example**: `[42, NULL, 30]`

```
Nullmap: 0b00000010  (bit 1 set → name is NULL)
Payload: [42][30]    (no bytes for name)
```

### API

**Encoding** (`Layout`):
- `Layout::encode_payload(values: &[Constant]) -> Vec<u8>` — non-null values
- `Layout::encode_payload_with_nulls(values: &[Option<Constant>]) -> Vec<u8>` — with NULLs
- `Layout::decode_field(payload: &[u8], field_name: &str) -> Option<Constant>` — dynamic offset scan

**Insert** (`HeapPageViewMut`):
- `insert_row_values(values: &[Constant]) -> SimpleDBResult<SlotId>`
- `insert_row_values_with_nulls(values: &[Option<Constant>]) -> SimpleDBResult<SlotId>`
- Each insert allocates `payload_len + UPDATE_SLACK (4 bytes)` so minor string growth fits in-place

**Update** (`HeapPageViewMut`):
- `update_field(slot, field_name, value: Option<Constant>) -> SimpleDBResult<()>` — read-modify-write; if re-encoded tuple doesn't fit, inserts redirect

**Read** (`LogicalRow`):
- `get_column(field_name) -> Option<Constant>` — scans null bitmap + preceding fields to find offset

### Layout Changes

`Layout` no longer stores `slot_size` or a precomputed `offsets: HashMap`. Instead:
- `column_index: HashMap<String, usize>` maps field name → schema ordinal
- `max_encoded_size() -> usize` gives worst-case payload bytes (used by BTree capacity checks and planner `records_per_block` estimates)

### Space Savings

For a schema `{id: INT, name: VARCHAR(100)}` inserting "hi" (2 chars):
- Dense payload: 1 (bitmap) + 4 (int) + 4 (len) + 2 = 11 bytes
- Old fixed-slot payload: 4 (base) + 4 (int) + 4 + 100 (string max) = 112 bytes
- Saving: 101 bytes per tuple (90% reduction for this field)
