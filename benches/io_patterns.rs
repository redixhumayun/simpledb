...
fn bench_phase1_io(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("Phase1/IO Patterns");
    ...
    // Sequential Read
    {
        ...
        group.bench_function("Sequential Read", |b| {
            b.iter(|| {
                let mut page = Page::new();
    ...
    // Sequential Write
    {
        ...
        group.bench_function("Sequential Write", |b| {
            b.iter(|| {
                let mut page = Page::new();
                for i in 0..total_ops {
...
fn bench_durability(c: &mut Criterion) {
    ...
    group.throughput(Throughput::Elements(durability_ops as u64));

    for (wal_template, wal_name) in [(WALFlushPolicy::Immediate, "wal-immediate")] {
...
fn bench_cache_adverse(c: &mut Criterion) {
    ...
    // One-pass sequential scan
    {
        ...
        group.bench_function("One-pass Seq Scan", |b| {
            b.iter(|| {
                let mut page = Page::new();
                for i in 0..ws {
                    db.file_manager
    ...
    // Low-locality random read
    {
        ...
        group.bench_function("Low-locality Rand Read", |b| {
            b.iter(|| {
    ...
    // Multi-stream scan
    {
        ...
        group.bench_function("Multi-stream Scan", |b| {
            b.iter(|| {
...
fn bench_cache_evict(c: &mut Criterion) {
    ...
    // One-pass sequential scan + evict
    {
        ...
        group.bench_function("One-pass Seq Scan+Evict", |b| {
            b.iter_custom(|iters| {
    ...
    // Low-locality random read + evict
    {
        ...
        group.bench_function("Low-locality Rand Read+Evict", |b| {
            b.iter_custom(|iters| {
                let mut total = Duration::ZERO;
                for _ in 0..iters {
                    let mut indices: Vec<usize> = (0..ws).collect();
    ...
    // Multi-stream scan + evict
    {
        ...
        group.bench_function("Multi-stream Scan+Evict", |b| {
            b.iter_custom(|iters| {
                let mut total = Duration::ZERO;
                for _ in 0..iters {
                    let t = Instant::now();
