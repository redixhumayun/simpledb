...
def main() -> None:
    ...
    out_path = REPO_ROOT / "benches" / "REPLACEMENT_POLICIES.md"
    lines: List[str] = [
        "# Replacement Policy Comparison",
        "",
        "_Generated by `scripts/bench/render_replacement_policy_docs.py`."
        " Run that script to refresh after new benchmark runs._",
        "",
    ]
    ...
    for platform in args.platforms:
        ...
        heading = metadata.get("title", platform.title())
    ...
    lines += [
        "_Notes_:  ",
        "- Times are means from Phase 1 latency benches. Throughputs are means from Phase 2 "
        "(Repeated/Random) and Phase 5 (pin:t2/pin:t8 plus the pin:t16/hot-set:t16 oversubscription cases).  ",
